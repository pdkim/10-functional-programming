...
(function (module){
  ...
  */
  ...
  Article.numWordsAll = () => {
    ...
    //   return map;
    // }, Object.create(null));
    return Article.all.map(art => art.body.split(' ').length).reduce((acc, current) => acc + current)
  };
  ...
  Article.numWordsByAuthor = () => {
    return Article.allAuthors().map(author => {
      return {
        author,
        totalWords: Article.all.filter(article => article.author === author).map(art => art.body.split(' ').length).reduce((acc, current) => acc + current)
      }
    })
  };
